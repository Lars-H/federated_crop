...
class PhysicalPlan(object):
    ...
    def join_subplans(self, left, right, join_type=None, card=-1):
    ...
    def from_logical_plan(self, logical_plan):
        if isinstance(logical_plan, LogicalPlan):
            if len(logical_plan.filters) > 0:
                ...
                return self.from_logical_join(logical_plan[0], logical_plan[1], logical_plan[2], logical_plan.cardinality)
    ...
    def from_logical_join(self, left, right, join_type, card=0):
        if isinstance(left, TriplePattern) or isinstance(left, BGP):
            return self.create_triple_pattern_plan(left)

        elif left.is_triple_pattern and right.is_triple_pattern:
            return self.join_subplans(left[0], right[0], join_type=join_type, card=card)
            ...
            return self.join_subplans(self.from_logical_plan(left), self.from_logical_plan(right), join_type=join_type, card=card)