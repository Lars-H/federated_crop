...
def get_metadata_tpf_stats(servers, query):
    ...
    for server in servers:
        ...
        if response.status_code == 200:
            ...
            if len(matches) == 1:
                ...
                sum += total
                if total > 0:
                    query.sources[server] = total
                    myres = eval(res)
                    stats[server] = get_authorities(server,myres["@graph"], myres["@context"], query)
        else:
            print "Could not contact TPF server {}. Status code: {}".format(server, response.status_code)
...
def contact_single_tpf_server_binding(server, triple_pattern, queue, binding, vars=None):
    ...
    contact_single_tpf_server(server, tp, queue, vars, binding)
