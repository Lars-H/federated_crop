...
def contact_source_bindings(servers, query, queue, bindings, vars=None, **kwargs):
    ...
    for server in servers:
        if server in query.sources.keys():
            if "sparql@" in server:
                server_url = server.replace("sparql@", "")
                contact_single_endpoint_server_bindings(server_url, query, queue, bindings, vars, limit=sparql_limit)
            elif "brtpf@" in server:
                server_url = server.replace("brtpf@", "")
                contact_single_brtpf_server_binding(server_url, query, queue, bindings, vars)
            elif "tpf@" in server:
                server_url = server.replace("tpf@", "")
                contact_single_tpf_server_binding(server_url, query, queue, bindings, vars)
            else:
                contact_single_tpf_server_binding(server, query, queue, bindings, vars)
    queue.put("EOF")